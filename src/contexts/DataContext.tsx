...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    // Always deduplicate transactions by ID to strictly prevent any duplicate rows in UI
    const setTransactions: React.Dispatch<React.SetStateAction<Transaction[]>> = useCallback((action) => {
                ...
                .on('postgres_changes', { event: '*', schema: 'public', table: 'transactions' }, (payload) => {
                    if (payload.eventType === 'INSERT' || payload.eventType === 'UPDATE') {
                        ...
                        setTransactions(prev => {
                            ...
                            // Replace any temporary optimistic transaction that matches this incoming transaction
                            const tempIndex = prev.findIndex(t =>
                                (t.id.startsWith('TRN-PAY-') || t.id.startsWith('TRN-DP-')) &&
                                t.projectId === normalized.projectId &&
                                Math.abs(t.amount - normalized.amount) < 0.01
                            );
                            if (tempIndex !== -1) {
                                const copy = [...prev];
                            ...
                            return [normalized, ...prev].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
                        });
                    }
