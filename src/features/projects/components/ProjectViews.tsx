            ...
            <div className="hidden md:block overflow-x-auto w-full">
                <table className="w-full text-xs lg:text-sm text-left">
                    <thead className="text-[10px] lg:text-xs text-brand-text-secondary uppercase">
                        <tr>
                            <th className="px-2 lg:px-4 py-3 lg:py-4 font-medium tracking-wider text-center w-8 lg:w-12">No</th>
                            <th className="px-2 lg:px-6 py-3 lg:py-4 font-medium tracking-wider">Nama Acara Pernikahan</th>
                            <th className="px-2 lg:px-6 py-3 lg:py-4 font-medium tracking-wider">Pengantin</th>
                            <th className="px-2 lg:px-6 py-3 lg:py-4 font-medium tracking-wider">Tanggal</th>
                            <th className="px-2 lg:px-6 py-3 lg:py-4 font-medium tracking-wider min-w-[120px] lg:min-w-[200px]">Progress</th>
                            <th className="px-2 lg:px-6 py-3 lg:py-4 font-medium tracking-wider">Tim</th>
                            <th className="px-2 lg:px-6 py-3 lg:py-4 font-medium tracking-wider text-center">Aksi</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-brand-border">
                        {projects.map((p, index) => (
                            <tr key={p.id} className="hover:bg-brand-bg transition-colors">
                                <td className="px-2 lg:px-4 py-3 lg:py-4 text-center font-medium text-brand-text-secondary">{index + 1}</td>
                                <td className="px-2 lg:px-6 py-3 lg:py-4">
                                    <div className="flex items-center gap-2">
                                        <p className="font-semibold text-brand-text-light">{p.projectName}</p>
                                    </div>
                                    <p className={`text-[10px] lg:text-xs font-medium px-1.5 lg:px-2 py-0.5 rounded-full inline-block mt-1 ${getStatusClass(p.status, config)}`}>
                                        {getSubStatusText(p)}
                                    </p>
                                </td>
                                <td className="px-2 lg:px-6 py-3 lg:py-4 text-brand-text-primary">{p.clientName}</td>
                                <td className="px-2 lg:px-6 py-3 lg:py-4 text-brand-text-primary whitespace-nowrap">{new Date(p.date).toLocaleDateString('id-ID', { year: 'numeric', month: 'short', day: 'numeric' })}</td>
                                <td className="px-2 lg:px-6 py-3 lg:py-4">
                                    <div className="flex items-center gap-1.5 lg:gap-3">
                                        <ProgressBar progress={getDisplayProgress(p, config)} status={p.status} config={config} />
                                        <span className="text-[10px] lg:text-xs font-semibold text-brand-text-secondary min-w-[24px] lg:min-w-[28px] text-right">{getDisplayProgress(p, config)}%</span>
                                    </div>
                                </td>
                                <td className="px-2 lg:px-6 py-3 lg:py-4 text-brand-text-primary max-w-[100px] lg:max-w-none truncate">{p.team.map(t => t.name.split(' ')[0]).join(', ') || '-'}</td>
                                <td className="px-2 lg:px-6 py-3 lg:py-4">
                                    <div className="flex items-center justify-center space-x-1 lg:space-x-1.5">
                                        <button onClick={() => handleOpenDetailModal(p)} className="btn-box-read w-7 lg:w-8 h-7 lg:h-8 rounded-md lg:rounded-lg flex items-center justify-center" title="Detail Acara Pernikahan"><EyeIcon className="w-3.5 lg:w-4 h-3.5 lg:h-4 text-white flex-shrink-0" /></button>
                                        <button onClick={() => handleOpenForm('edit', p)} className="btn-box-edit w-7 lg:w-8 h-7 lg:h-8 rounded-md lg:rounded-lg flex items-center justify-center" title="Edit Acara Pernikahan"><PencilIcon className="w-3.5 lg:w-4 h-3.5 lg:h-4 flex-shrink-0" /></button>
                                        <button onClick={() => handleProjectDelete(p.id)} className="btn-box-delete w-7 lg:w-8 h-7 lg:h-8 rounded-md lg:rounded-lg flex items-center justify-center" title="Hapus Acara Pernikahan"><Trash2Icon className="w-3.5 lg:w-4 h-3.5 lg:h-4 text-white flex-shrink-0" /></button>
                                    </div>
                                </td>
                            </tr>
                        ))}
